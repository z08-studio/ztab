...
Verified on 2026-09-18 in an isolated Chrome for Testing profile with the actual extension page: **Support my coding** opens the dialog with **Connect wallet** as its primary action. Presets and custom amounts update the checkout URL, and invalid amounts hide the link. **Scan or copy address** expands the manual fallback; copying uses the configured recipient. Escape closes the dialog and returns focus to the footer. Light and dark layouts fit 280px, 320px, and 420px widths without horizontal overflow. The unsupported MetaMask mobile links are absent from both interfaces.

The extension-to-checkout flow was tested with the proposed GitHub Pages URL routed to the local production build in the isolated browser: selecting 10 USDC opens the expected URL, retains the selected amount, and opens the real Reown wallet picker. The temporary test route was removed afterward. This verifies local navigation and integration, not public deployment; the GitHub Pages site is awaiting hosting approval and must be published before this package is distributed.

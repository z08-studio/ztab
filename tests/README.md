...
The standalone support page was reviewed in light and dark styles at 320px, 420px, and 900px. The URL amount, custom validation, clipboard success, and denied-clipboard manual selection worked. Actual rendered QR images for 3 and 1.234567 USDC decoded to the expected Base payment request. Without a Reown Project ID, Connect is disabled, the fallback is visible, and loading the page makes no requests to external services. The production build rejects missing configuration; a separate local preview build succeeds.

With the supplied Project ID in an ignored local environment file, the production build succeeds. The real AppKit picker loads wallet options including MetaMask, Coinbase, and Trust Wallet. WalletConnect produces a pairing QR; no wallet was paired and no transaction was sent. The integration uses the installed SDK's `getAccount().isConnected` API when choosing the Connect or Account view.
